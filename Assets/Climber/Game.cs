...
namespace Climber
{
    public class Game
    {
        ...
        public void SetupCollisionTable()
        {
            ...
            RegisterCollision(ObjType.Bullet, ObjType.Player, (ref Contact c) =>
            {
                // do colision betweem e.A bullet, and e.B enemy!            
                if (c.impulse > 0)
                {
            ...
            RegisterCollision(ObjType.Explosion, ObjType.Player, (ref Contact c) =>
            {
                //if (c.impulse > 0)
            ...
            RegisterCollision(ObjType.Tilemap, ObjType.Explosion, (ref Contact c) =>
            {
                if (c.dq.flags.Contains(DistanceQuery.Touch) == true)
                {
                    // do colision betweem e.A bullet, and e.B enemy!
                    ref var tm = ref Components._Tilemap[c.A];
                    ref var tl = ref tm.tiles[c.TileY * tm.width + c.TileX];
                    if (tl == 1)
                    {
                        tl = 0;
                    }
        ...
        {
            var iterations = Static.Const.Physics.Iterations;
            var bias = Static.Const.Physics.Bias;
            var slop = Static.Const.Physics.Space;
        ...
        {
            var bounce = Static.Const.Physics.Bounce;
                                ...
                                for (short row = sy; row <= ey; row++)
                                {
                                    for (short col = sx; col <= ex; col++)
                                    {
                                        ...
                                        else
                                        {
                                            if (t == 0 && dead == true)
                                            {
                                                tiles.tiles[row * tiles.width + col] = 3;
                                            }
                                        }
                                    }
                                }
        ...
        public void Gravity()
        {
            for (var r = Components.Filter(Cp.Active | Cp.Body); r.MoveNext();)
            {
                if (r.components.Contains(Cp.Tilemap) == false)
                {
            ...
            for (int event_index = 0; event_index < Events.Size; ++event_index)
            {
                ...
                switch (evt.Id)
                {
                    ...
                    case EventType.Explode:
                        {
                            var r = Components.Get(evt.A);
                            var pos = r.body.position;
                            pos -= Components._Body[Global.Ref.worldMap].position;

                            ref var tiles = ref Components._Tilemap[Global.Ref.worldMap];

                            short sx, sy, ex, ey;

                            var lower = pos - new Vector2(64, 64);
                            var upper = pos + new Vector2(64, 64);
                            ...
                            for (short row = sy; row <= ey; row++)
                            {
                                for (short col = sx; col <= ex; col++)
                                {
                                    ...
                                    if (t == 1)
                                    {
                                        tiles.tiles[row * tiles.width + col] = 0;
                                    }
                                }
                            }
