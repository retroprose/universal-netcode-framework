...
namespace Climber
{
    ...
    public class ProgramVector : Vector<Program>
    {
        ...
        {
            ref var p = ref m_list[index];
            if (p.Function.PreviousId != 0xffff)
            {
    ...
    public struct GrenadeFunction : FunctionUnion.IFunction
    {
        public FunctionUnion.Id Id => FunctionUnion.Id.GrenadeFunction;
        public void Make(ref FunctionUnion u) => u.grenade = this;

        public byte Counter;

        public GrenadeFunction(byte n)
        {
            Counter = n;
        }
        ...
        public bool Invoke(Game g, ref Cp.Ref r)
        {
            ...
            if (Counter == 0)
            {
                ...
                {
                    g.Events.Last.DestroyEntity(e);
                }
                if (g.Events.Append() == true)
                {
                    g.Events.Last.CreateEntity(ObjType.Explosion, r.body.position, new Vector2(0, 0));
                }
                /*if (g.Events.Append() == true)
                {
                    g.Events.Last.Explode(e, 50, 50);
                }*/
            }   
            return false;
        }
    ...
    public struct DestroyFunction : FunctionUnion.IFunction
    {
        public FunctionUnion.Id Id => FunctionUnion.Id.DestroyFunction;
        public void Make(ref FunctionUnion u) => u.destroy = this;
    ...
    public struct PlayerFunction : FunctionUnion.IFunction
    {
        ...
        public bool Invoke(Game g, ref Cp.Ref r)
        {
            ...
            if (r.player.dead == false)
            {
                ...
                r.body.velocity.x = move.x * 10;

                if (Cooldown > 0)
                    Cooldown--;

                if (Cooldown == 0 && slot.Up)
                {
                    r.body.velocity.y += 10;
                    Cooldown = 20;
                }
                ...
                if (slot.Primary)
                {
                    if (g.Events.Append() == true)
                    {
                        g.Events.Last.CreateEntity(ObjType.Bullet, r.body.position + diff * 40, diff * 30);
                    }
                }
                ...
                if (GrenadeCooldown == 0 && slot.Secondary)
                {
                    GrenadeCooldown = 25;
                    if (g.Events.Append() == true)
                    {
                        g.Events.Last.CreateEntity(ObjType.Grenade, r.body.position + diff * 20, diff * 20);
                    }
                }
    ...
    public struct FunctionUnion
    {
        ...
        */
        ...
        [FieldOffset(0)] public GrenadeFunction grenade;
        [FieldOffset(0)] public DestroyFunction destroy;
        ...
        public bool Invoke(Id id, Game g, ref Cp.Ref r)
        {
            switch (id)
            {
                ...
                case Id.GrenadeFunction: return grenade.Invoke(g, ref r);
                case Id.DestroyFunction: return destroy.Invoke(g, ref r);
