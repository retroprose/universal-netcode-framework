...
public class ClimberLoad
{
    public static Dictionary<ObjType, Cp.Prefab> Load(JObject objects)
    {
        ...
        {
            Log.ForceLine($"{property.Name} - {property.Value.Type}");
        }
            ...
            if (o.TryGetValue("Program", out t))
            {
                ...
                if (c.TryGetValue("id", out m))
                {
                    ...
                    switch (id)
                    {
                        ...
                        case "GrenadeFunction":
                            p.program.Function.FunctionId = (ushort)FunctionUnion.Id.GrenadeFunction;
                            p.program.Function.Union.grenade = new GrenadeFunction();
                            if (c.TryGetValue("data", out n))
                            {
                                p.program.Function.Union.grenade = new GrenadeFunction((byte)n.Value<int>());
                            }
                            break;

                        case "DestroyFunction":
                            p.program.Function.FunctionId = (ushort)FunctionUnion.Id.DestroyFunction;
                            p.program.Function.Union.destroy = new DestroyFunction();
                            break;
...
public class ClimberIO : BaseIO
{
    static public GameId _Id = GameId.Climber;
    ...
    public override void Init(GameObject c, sbyte l, PassedData p)
    {
        ...
        TextAsset ta = (TextAsset)Resources.Load("Data\\platformer");
        ...
        TextAsset ta_all = (TextAsset)Resources.Load("Data\\climber_objects");
        var obj_all = JsonConvert.DeserializeObject<JObject>(ta_all.text);
        Static.Const.Objects = ClimberLoad.Load(obj_all["Objects"].Value<JObject>());

        Static.Const.Physics = new Data.PhysicsData();
        Static.Const.Physics.Bias = (FixedMath.Scaler)(obj_all["Physics"]["Bias"].Value<float>());
        Static.Const.Physics.Bounce = (FixedMath.Scaler)(obj_all["Physics"]["Bounce"].Value<float>());
        Static.Const.Physics.Extend = (FixedMath.Scaler)(obj_all["Physics"]["Extend"].Value<float>());
        Static.Const.Physics.Iterations = obj_all["Physics"]["Iterations"].Value<int>();
        Static.Const.Physics.Overlap = (FixedMath.Scaler)(obj_all["Physics"]["Overlap"].Value<float>());
        Static.Const.Physics.Space = (FixedMath.Scaler)(obj_all["Physics"]["Space"].Value<float>());
    ...
    public override void ProcessOutput()
    {
        ...
        for (var r = cp.Filter(Cp.Active | Cp.Body); r.MoveNext();)
        {
                ...
                for (short row = sy; row <= ey; row++)
                {
                    for (short col = sx; col <= ex; col++)
                    {
                        ...
                        */
                        ...
                        t = c = 0;
                        if (f == 1) { t = (int)'#'; c = 7; }
                        else if (f == 2) { t = (int)'#'; c = 8; }
                        else if (f == 3) { t = 176; c = 4; }
                        else { }
            ...
            else
            {
                ...
                if (r.objectId == (int)ObjType.Grenade) { t = 254; c = 10; }
                if (r.objectId == (int)ObjType.Explosion) { t = 15; c = 14; }
